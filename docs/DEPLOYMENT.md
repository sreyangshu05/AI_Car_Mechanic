...
3. In Render, set `SECRET_KEY` to a new random value, `DEBUG=False`, and `CORS_ALLOWED_ORIGINS=https://<your-frontend>.vercel.app`. The app automatically adds Render's `RENDER_EXTERNAL_HOSTNAME` to Django's `ALLOWED_HOSTS`. If that variable is unavailable for your service, set `ALLOWED_HOSTS=ai-car-mechanic.onrender.com` (or your exact Render hostname), comma-separated with any other required hosts.
